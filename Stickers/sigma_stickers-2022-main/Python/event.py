...
class Time(object):
	...
	def __lt__(self, other):
		...
		selfInt =  (int(time.strftime('%H%M', time.strptime(selfstring, '%I:%M %p'))) - 300)%2400
		otherInt =  (int(time.strftime('%H%M', time.strptime(otherstring, '%I:%M %p'))) - 300)%2400
	...
	def __eq__(self, other):
		if self.noTime:
			return False
		if self.ampm == other.ampm and self.hour == other.hour and self.min == other.min:
			return True
		return False
...
class Event(object):
	...
	def __init__(self, name, day, time, category): #time is a tuple of times - start, end
