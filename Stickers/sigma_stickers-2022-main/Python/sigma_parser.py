...
class SigmaParser:
    ...
    def __init__(self):
        ...
        self.STBrackets = []	#array of class STBRacket with all SigmaTournament Brackets
        self.camperInfo = []
    ...
    def extractDiningHallDuties(self):
        ...
        for row in range(1,len(data)):
            for column in range(1,len(data[row])):
                ...
                if team != "":
                    self.teams[team].add_event(Event("Team $\\" + team + "$ on " + meal + " duty", day, mealtime, "teamdininghall"))
    ...
    def extractTeamDuties(self):
        ...
        for row in range(1,len(data)):
            for column in range(1,len(data[row])):
                ...
                if team != "":
                    self.teams[team].add_event(Event("Team $\\"+ team+"$ cleanup in "+location , day, timeCleanup, "teamcleanup"))
    ...
    def extractSemilabs(self):
        ...
        for row in data:
            ...
            self.semilabs.append(Semilab(semilabName, semilabShortName, timeofday, timeSlot))
            ...
            for name in instructors:
                if self.staff[name].role == "faculty":
                    for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]:
                        self.staff[name].add_event(Event("Instructor for "+semilabShortName, day, timeSlot, "semilab"))
                self.staff[name].add_task(Task("Instructor for "+semilabShortName+ " in the "+ timeofday, "semilab"))

            for name in teachingAssistants:
                if self.staff[name].role == "faculty":
                    for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]:
                        self.staff[name].add_event(Event("TA for "+semilabShortName, day, timeSlot, "semilab"))
                self.staff[name].add_task(Task("TA for "+semilabShortName+ " in the "+ timeofday, "semilab"))
    ...
    def extractTournament(self):
        ...
        for row in range(1, len(data)):
            for col in range(1, len(headerDays)):
                if data[row] != [] and data[row][col] != "":
                    ...
                    self.staff[name].add_event(Event("Tournament " + position + " at " + location, day, timeTournament, "tournament"))
                    
                    if position == "Head Judge":
                        self.staff[name].add_event(Event("Judges Meeting (head judge must attend)", day, timeMeeting, "tournament"))

                    else:
                        self.staff[name].add_event(Event("Judges Meeting (optional)", day, timeMeeting, "tournament"))
        ...
        for row in range(1,len(data)):
            for i in range((len(data[row])-1)/2):
                ...
                if data[row][col1] != "" and data[row][col2] != "":
                    ...
                    team1 = data[row][col1]
                    team2 = data[row][col2]
                    self.STBrackets.append(STBracket(team1, team2, location, day, locationabbreviations[location]))
        ...
        for row in data:
            if len(row) == 2:
                if row[0] in self.teams and row[1] != "":
                    self.staff[row[1]].add_event(Event("Timer at GU with $\\"+row[0]+"$ at "+locationGradUni, dayGrandUnification, timeTournamentGrandUni, "tournament"))
                    self.staff[row[1]].add_event(Event("GU Meeting at " + locationMeetingGrandUni, dayMeetingGrandUni, timeMeetingGrandUni, "tournament"))
        ...
        for row in data:
            if row != []:
                self.staff[row[0]].add_event(Event("Runner at GU at"+locationGradUni, dayGrandUnification, timeTournamentGrandUni, "tournament"))
        ...
        for row in data:
            if row != []:
                self.staff[row[0]].add_event(Event("Grader at GU at "+locationGradUni, dayGrandUnification, timeTournamentGrandUni, "tournament"))
                self.staff[row[0]].add_event(Event("GU Meeting at " + locationMeetingGrandUni, dayMeetingGrandUni, timeMeetingGrandUni, "tournament"))
        ...
        for row in data:
            ...
            if len(row) > 1:
                ...
                if instructor1 in self.staff:
                    self.staff[instructor1].add_task(Task("Workshop "+row[0], "workshop"))
                if instructor2 in self.staff:
                    self.staff[instructor2].add_task(Task("Workshop "+row[0], "workshop"))
                    ...
                    if len(data[rowId]) > colId:
                        ...
                        if not workshopName in workshops:
                            ...
                            if instructor1 in self.staff:
                                ...
                                self.staff[instructor1].add_event(Event("Workshop: " + workshopName + ", location: " + location, day, time, "workshop"))

                                dayIndex = self.daysOfWeek.index(day)
                                if (dayIndex - 1) >= 0:
                                    self.staff[instructor1].add_event(Event("Workshop tomorrow:  " + workshopName, self.daysOfWeek[dayIndex-1], "NA", "personalreminder"))

                            else:
                                print(instructor1 + ", workshop instructor, not in staff list")
                            if instructor2 is not None:
                                if instructor2 in self.staff:

                                    self.staff[instructor2].add_event(Event(workshopName + " workshop at " + location, day, time, "workshop"))

                                    if (dayIndex - 1) >= 0:
                                        self.staff[instructor2].add_event(Event("Workshop tomorrow:  " + workshopName, self.daysOfWeek[dayIndex-1], "NA", "personalreminder"))

                                else:
                                    print(instructor2 + ", workshop instructor, not in staff list")
        '''
    ...
    def extractNightShift(self):
        ...
        for rowi in range(2, len(data)):
            for colj in range(1, len(data[rowi])):
                ...
                if name != "":
                    self.staff[name].add_event(Event("Night Shift: " + shift + " at " + location + " (" + other_name + " on " + other_shift + ")", day, time, "nightshift"))
                ...
                for name in self.staff:
                    if self.staff[name].role == "counselor":
                        if self.staff[name].team == team:
                            self.staff[name].add_event(Event(nameOnDuty + " stop by counselor office hours", day, time, "counselor office hours"))
            ...
            for counselor in counselors:
                for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]:
                    if counselor == counselorName:
                        self.staff[counselor].add_event(Event("Take " + camperName + " to nurse", day, nursetimes[timeOfDay], "nurse"))
                    else:
                        self.staff[counselor].add_event(Event(counselorName + " takes " + camperName + " to nurse", day, nursetimes[timeOfDay], "nurse"))
        ...
        for rowi in range(1, len(data)):
            for colj in range(1, len(data[rowi])):
                ...
                if reminder != "":
                    day = data[0][colj]
                    team = data[rowi][0]
                    self.teams[team].add_event(Event(reminder, day, "NA", "teamreminder"))
        ...
        for rowi in range(1, len(data)):
            for colj in range(1, len(data[rowi])):
                ...
                if reminder != "":
                    ...
                    for name in self.staff:
                        if self.staff[name].role == "counselor":
                            self.staff[name].add_event(Event(reminder, day, "NA", category+"reminder"))
            ...
            for colj in range(4, len(data[rowi])):
                ...
                if name != "":
                    ...
                    if location == "":
                        ...
                        self.staff[name].add_event(Event(duty + " at " + location, day, time, "duty"))
    ...
    def extractCamperInfo(self):
        ...
        for row in data:
            ...
            semilab1 = row[9]
            semilab2 = row[10]
            self.camperInfo.append(Camper(namefirst, namelast, age, past_camps, team, semilab1, semilab2, building, nurse, epi))
