...
def createtournamenttable(outfile, brackets):
    if len(brackets) != 7:
        if len(brackets) == 0:
            ...
            raise Exception("Tournament table must have length 7. Length is " + str(len(brackets)))
    ...
    for name in mydata.staff:
        if mydata.staff[name].role == "counselor":
            ...
            if team is not None:
                ...
                outfile.w("\\textbf{Name} & \\textbf{Age} & \\textbf{Morning} & \\textbf{Afternoon} \\\\ \n \\hline\n")
                ...
                for i in range(len(camperlist)):
                    ...
                    outfile_string = camperlist[i].namefirst + " " + camperlist[i].namelast + camperSuperscript + "&" + camperlist[i].age + "&" + camperlist[i].semilab1.replace("_", " ")[:13] + "&" + camperlist[i].semilab2.replace("_", " ")[:13]
...
def generate_camper_info_stickers_lena():
    ...
    for team in mydata.teams:
        ...
        stickerheader = counselorstring + "{\\LARGE $\\" + team + "$}" + " (" + building + ")"
...
def generate_team_schedule():
    ...
    for team in mydata.teams:
        ...
        for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]:
            counter = counter + 1
            today_events = [event for event in all_events if event.day == day and event.timeStart != "NA"]
            ...
            if len(ordered_events) != 0:
                ...
                for myevent in ordered_events:
                    ...
                    outfile.wl("\\event{\\Large "+str(myevent.timeStart)+"}{\\Large "+myevent.name+"} \\\\")
        outfile.wl("\\newpage")
...
def createCounselorSticker(name, day, mydata, outfile):
    ...
    morningreminders = [event.name for event in mydata.staff[name].events if event.category == "morningreminder" and event.day == day]
    afternoonreminders = [event.name for event in mydata.staff[name].events if event.category == "afternoonreminder" and event.day == day]
    eveningreminders = [event.name for event in mydata.staff[name].events if event.category == "eveningreminder" and event.day == day]
    teamreminders = [event.name for event in mydata.staff[name].events if event.category == "teamreminder" and event.day == day]
    personalreminders = [event.name for event in mydata.staff[name].events if event.category == "personalreminder" and event.day == day]
    
    outfile.wl(r'\begin{sticker}')

    #create tournament table
    tournamentBracketsToday = [bracket for bracket in mydata.STBrackets if bracket.day == day]
    ...
    if len(personalreminders) != 0:
        outfile.wl("\\header{"+"black"+"}{Personal Reminders}\n\\begin{stickerlist}")
    ...
    for entry in ordered_events:
        if entry.timeStart < morningRemindersTime:
            outfile.wl("\\scheduleentry{" + entry.timeStart.print_hour_and_minutes() + "}{" + entry.timeStart.ampm + "}{" + entry.name + "}")
    ...
    outfile.wl("\\begin{scheduletable}")
    for entry in ordered_events:
        if entry.timeStart > morningRemindersTime and entry.timeStart < afternoonRemindersTime:
            outfile.wl("\\scheduleentry{" + entry.timeStart.print_hour_and_minutes() + "}{" + entry.timeStart.ampm + "}{" + entry.name + "}")
    outfile.wl("\\end{scheduletable}")

    #afternoon reminders
    if len(afternoonreminders) != 0:
        outfile.wl("\\header{"+"green"+"}{Afternoon Reminders}\n\\begin{stickerlist}")
    ...
    outfile.wl("\\begin{scheduletable}")
    for entry in ordered_events:
        if entry.timeStart > afternoonRemindersTime and entry.timeStart < eveningRemindersTime:
            outfile.wl("\\scheduleentry{" + entry.timeStart.print_hour_and_minutes() + "}{" + entry.timeStart.ampm + "}{" + entry.name + "}")
    outfile.wl("\\end{scheduletable}")
...
def generate_semilab_camper_profiles():
    ...
    for semilab in mydata.semilabs:
        for camper in mydata.camperInfo:
            ...
            if camperSemilab.replace("_", " ") == semilab.nameShort: #and camper.namelast in camperlist:
                ...
                out_string = r'\stickertitle{' + campernamefirst + ' ' + campernamelast + r'}'
                outfile.wl(out_string.encode('utf-8').strip())
                semilabHeader = r'\stickertitle{\fontsize{10}{10}\selectfont '+ semilab.nameShort + ', ' + semilab.timeOfDay + r'}'
                ...
                out_string_photo = r'\noindent\begin{minipage}{0.5\textwidth}\includegraphics[width=1in]{camper_photos/' \
                    + filename + r'}\end{minipage}\begin{minipage}{0.45\textwidth}'
                ...
                otherSemilab = camper.semilab2 if semilab.timeOfDay == "morning" else camper.semilab1
...
def generate_custom_named_sticker(filename):
    ...
    for row in mydata.stickerPlacement:
        for i in range(len(row)):
            ...
            outfile.wl(r'\renewcommand{\baselinestretch}{0.8} \begin{sticker} \stickertitle{' + name + " ($\\" + mydata.staff[name].team + "$)" + r'}')
